...
- `mockgen -source=internal/config/db/interfaces.go -destination=internal/mocks/mock_db.go -package=mocks IPG` - пример генерации моков для тестов
- `set -a && source .env && set +a` - загрузить переменные окружения в терминал
- `sudo docker compose up -d` - В режиме демона запуск Postgres
- `go test ./... -coverprofile=coverage.out && go tool cover -html=coverage.out -o coverage.html` - отчёт по покрытию тестами
